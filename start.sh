...
then
 ...
 java  -Dlogging.config=./log4j2.xml -Dlog.level=${log_level} -Dlog.path=${log_path} -Dmodule.name=${module_name} -Dspring.config.location=file:./application.properties,file:${commonConfigurationFile}  -jar $build  1>/dev/null 2>${log_path}/${module_name}.error &
